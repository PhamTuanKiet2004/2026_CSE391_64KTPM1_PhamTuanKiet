...
+ Dòng Hello from JavaScript!
> - Điều này cho em biết JavaScript đang làm gì khi trang web được tải?
+ Trình duyệt đã tải và thực thi tệp main.js ngay khi trang web được mở lên
...
+ Trong bài này: JS chạy trên trình duyệt người dùng (Client, JS cũng có thể chạy trên Server với Node.js)
> b) HTML, CSS, JavaScript mỗi phần chịu trách nhiệm chính về điều gì?
>
> - HTML: Xây dựng bộ khung, cấu trúc và nội dung của trang web
> - CSS: Trang trí, tạo kiểu dáng, bố cục, màu sắc
> - JavaScript: Xử lý logic, tạo sự tương tác và hành vi động cho trang web.
...
>   - `number` là: kiểu số (bao gồm cả số nguyên và số thập phân).
>   - `string` là: kiểu chuỗi ký tự (văn bản), được đặt trong dấu ngoặc kép hoặc nháy đơn.
>   - `boolean` là: kiểu luận lý, chỉ có 2 giá trị là true (đúng) hoặc false (sai).
...
+ Dòng log hiển thị: Xin chào, mình là Kiệt, năm nay mình 22 tuổi.
> - Nếu em quên dấu `;` hoặc quên dấu `+`, điều gì xảy ra? Trình duyệt báo lỗi thế nào?
+ Quên ; thì trình duyệt thường vẫn tự thêm vào nên có thể không lỗi.
+ Quên + thì Console sẽ báo lỗi cú pháp chữ đỏ, ví dụ Uncaught SyntaxError: missing hoặc Unexpected identifier, do JS không hiểu cách nối các chuỗi lại với nhau.
...
+ Console không chỉ là nơi để trình duyệt thông báo lỗi, mà còn là một môi trường cho phép gõ và chạy trực tiếp các câu lệnh JavaScript. Có thể dùng nó như một chiếc máy tính bỏ túi hoặc để thử nghiệm nhanh chóng các đoạn code mà không cần phải viết vào file .js rồi tải lại trang.
> - Em gặp lỗi cú pháp nào? Em đã xử lý bằng cách nào (tự sửa, hỏi bạn, đọc lỗi, tìm Google, …)?
+ Lỗi gặp phải: quên đóng ngoặc kép khi nối chuỗi (ví dụ: console.log("Xin chào + name);), console báo dòng chữ đỏ Uncaught SyntaxError: Invalid or unexpected token.
+ Cách xử lý: Đọc thông báo lỗi trong tab Console để xem lỗi nằm ở file nào, dòng số mấy (ví dụ: main.js:5). Sau đó mở lại file code, tự sửa bằng cách thêm dấu ngoặc kép cho đúng cú pháp, lưu file và F5 reload lại trình duyệt để kiểm tra.
---
...
+ Khi giá trị của biến score nhỏ hơn 5.
> - Em hãy mô tả cấu trúc `if/else` bằng lời của em (có thể ví von “ngã rẽ” trong đời sống):
+ Giống như đi đến ngã tư. Nếu đèn xanh (điều kiện if đúng) thì được đi tiếp (thực thi code trong block if). Nếu đèn đỏ (điều kiện sai), phải dừng lại hoặc chuyển sang hướng rẽ khác (kiểm tra tiếp else if hoặc rơi vào else).
...
> Em ghi lại giá trị hàm trả về: kết quả trả về = 8
...
>   - Tên hàm: dùng để gọi hàm (ví dụ: tinhDiemTrungBinh)
>   - Tham số (parameters): dữ liệu đầu vào để hàm xử lý (ví dụ: m1, m2, m3)
>   - Thân hàm (body): các câu lệnh logic ằm trong cặp dấu {}.
>   - Giá trị trả về (return): kết quả đẩy ra sau khi xử lý (dùng từ khoá return)
> - Ưu điểm của việc dùng hàm thay vì lặp lại cùng một đoạn code nhiều lần là gì?
+ tái sử dụng code nhiều lần không cần viết lại, giúp chương trình ngắn gọn, dễ độc và dễ sửa lỗi (chỉ cần sửa ở một nơi).
...
+ Với if/else: thường hay nhầm lẫn giữa một dấu bằng = (dùng để gán giá trị) và hai/ba dấu bằng ==, === (dùng để so sánh). Hoặc lúng túng khi có quá nhiều nhánh else if lồng nhau.
+ Với hàm (function): Khái niệm return là khó hiểu nhất. Sinh viên thường không phân biệt được sự khác nhau giữa việc dùng console.log() (chỉ in chữ ra màn hình để xem) và return (hàm thực sự nhả ra một giá trị để gán vào biến khác và tính toán tiếp). Ngoài ra, việc truyền tham số (tham số hình thức lúc tạo hàm vs. giá trị thực tế lúc gọi hàm) cũng hay gây bối rối.
> - Em đã làm gì để vượt qua (thử nhiều lần, hỏi bạn, xem lại ví dụ, tra Google, …)?
+ Em đã thử thay đổi các con số truyền vào hàm (ví dụ đổi score thành các số âm, số lớn hơn 10) để xem mã code chạy vào nhánh if hay else nào.
---
...
+ Quét toàn bộ HTML và tìm ra thẻ có thuộc tính id="status" để đưa vào biến quản lý.
> - Sự kiện `"click"` xảy ra khi nào?
+ Khi người dùng click chuột trái vào phần tử đó (nút "Chào").
> - Trong đoạn code trên, khi nhấn nút `btnHello`, điều gì thay đổi trên trang?
+ Nội dung văn bản (textContent) của thẻ <p id="status"> sẽ bị đổi thành "Xin chào! Đây là nội dung được thay đổi bằng JavaScript."
---
...
+ Gán document.body.style.backgroundColor = "lightblue";
> - Em hãy ghi lại 1 ví dụ khác mà JavaScript có thể làm với `document.body.style`.
+ document.body.style.color = "blue"; hoặc ẩn phần tử document.body.style.display = "none";
---
...
+ "click" chỉ chạy một lần khi nhấn chuột. "input" chạy liên tục mỗi khi giá trị trong ô nhập liệu bị thay đổi (gõ thêm, xóa bớt, paste chữ).
> - Khi em xoá hết nội dung ô input, dòng `greeting` hiển thị gì?
+ Dòng greeting hiển thị "Xin chào, !" (do biến value lúc này là một chuỗi rỗng "").
...
> + DOM giống như một cái cây gia phả. Trình duyệt dịch các thẻ HTML thành các "nhánh cây" (object) để JavaScript có thể tìm đến từng nhánh, hái lá (lấy dữ liệu), hoặc gắn thêm hoa (thay đổi giao diện/nội dung).
> - Nêu 1 ví dụ “thao tác DOM” trong bài (ghi lại 1 dòng lệnh cụ thể).
+ Lệnh greeting.textContent = "Xin chào, " + value + "!";
...
+ Đều lắng nghe hành động click lên một nút có ID là btnHello và hiển thị một thông báo bật lên (alert).
> - Điểm khác nhau về cú pháp là gì (`document.getElementById` vs `$("#id")`, `addEventListener` vs `.on`)?
+ Tìm phần tử: JS dùng document.getElementById("btnHello") (dài), jQuery dùng $("#btnHello") (ngắn gọn, giống selector CSS).
+ Gắn sự kiện: JS dùng .addEventListener("click", ...), jQuery dùng .on("click", ...).
> - Em hãy tra cứu nhanh “What is jQuery used for?” và ghi 2 ý chính:
>   1. Giúp thao tác DOM, xử lý sự kiện (event handling) và gọi Ajax dễ dàng, ngắn gọn hơn rất nhiều so với JS thuần.
>   2. Xử lý các vấn đề không tương thích giữa các trình duyệt khác nhau (cross-browser compatibility).
...
+ Cách JavaScript tạo ra các hiệu ứng chuyển động mượt mà (animation) mà không cần dùng CSS.
+ Cách lưu trữ lại dữ liệu người dùng (ví dụ: gõ tên vào ô input, tắt trình duyệt đi mở lại vẫn còn tên ở đó).
+ Cách kết nối với các hệ thống dữ liệu bên ngoài (API) để lấy thông tin tự động thay vì gõ tay.
> 2. Em muốn tự làm thêm tính năng gì trên trang web (vd: bộ đếm, đổi theme, pop-up, mini game, …)?
+ Làm một bộ đếm số lượng (ví dụ: nút (+) (-) để tăng giảm số lượng sách khi thêm vào giỏ hàng).
+ Làm form đăng nhập cơ bản: tự động hiện chữ đỏ cảnh báo nếu người dùng nhập sai mật khẩu hoặc để trống thông tin.
